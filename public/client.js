  ...
  if (currentTrip.savedPlaces.length > 0) {
    for (let index = 0; index < currentTrip.savedPlaces.length; index++) {
      ...
      placeHTML.push(`<div class="saved-place" data-place-id="${place.id}">
            ${
              shouldEdit
                ? `<button class="js-edit place edit-place" aria-label="edit ${place.name} place"><i class="far fa-edit"></i></button>
...
function displayAIGeneratedTrip(currentTrIP) {
  ...
  fetch("https://api.openai.com/v1/completions", {
    ...
    .then((result) => {
      ...
      const ele = document.querySelector(".ai-generated");
      if (ele) {
...
function displayAIGeneratedTrip2(currentTrIP) {
  ...
  fetch("https://api.openai.com/v1/completions", {
    ...
    .then((result) => {
      ...
      const completion = result.choices[0].text;
      console.log(completion);
      const ele = document.querySelector(".ai-generated-2");
      if (ele) {
        ele.innerHTML = completion;
      }
    })
...
function displaySelectedTrip(currentTrip, shouldEdit) {
  ...
  $("#current-trip").html(`<div id="trip-details">
    ...
    <div>AI recommended travel itenary</div>
    <div class="ai-generated">
    <div class="loader"></div>
    </div>
    ${setTimeout(displayAIGeneratedTrip(currentTrip), 10000)}
    <div class="ai-generated-2">
    <div class="loader"></div>
    </div>
    ${setTimeout(displayAIGeneratedTrip2(currentTrip), 10000)}
    <div class="ai-generated-3">
    <div class="loader"></div>
    </div>
    ${setTimeout(displayAIGeneratedTrip3(currentTrip), 10000)}
    `);
...
function displayOneTrip(currentTrip) {
  $("#active-trips")
    .append(`<div class="trip-preview" data-id=${currentTrip.id}>
...
function displayActiveTrips(responseJson) {
  ...
  $("#active-trips")
    .append('<button class="add-trip">Add New Trip</button>')
    .prop("hidden", false);
